...
function toComma(n: number | null): string {
  return n !== null ? n.toLocaleString('ko-KR') : '';
}

function colorIndicesFromOptions(options: { name: string; value: string }[]): number[] {
  const indices: number[] = [];
  for (const opt of options) {
    const idx = COLORS.findIndex((c) => c.name === opt.value);
    if (idx !== -1 && !indices.includes(idx)) indices.push(idx);
  }
  return indices;
}

export default function ProductEditClient({ initial }: { initial: ProductEditData }) {
  const router = useRouter();

  const [selectedColors, setSelectedColors] = useState<number[]>(colorIndicesFromOptions(initial.options));
  const [priceValue, setPriceValue] = useState<string>(toComma(initial.price));
  const [welfareValue, setWelfareValue] = useState<string>(toComma(initial.welfarePrice));
  const [supplierValue, setSupplierValue] = useState<string>(toComma(initial.supplierPrice));
  const [pointValue, setPointValue] = useState<string>(initial.pointRate !== null ? String(initial.pointRate) : '');
  const [autoCalc, setAutoCalc] = useState<boolean>(false);
  const [mainImage, setMainImage] = useState<{ file: File | null; preview: string } | null>(
    initial.thumbnail ? { file: null, preview: initial.thumbnail } : null
  );
  const [subImages, setSubImages] = useState<SubImage[]>(() => {
    const slots: SubImage[] = [null, null, null, null, null];
    initial.images
      .slice()
      .sort((a, b) => a.order - b.order)
      .slice(0, 5)
      .forEach((img, idx) => {
        slots[idx] = { file: null, preview: img.url };
      });
    return slots;
  });
  const [exposeOn, setExposeOn] = useState<boolean>(initial.status === 'published');
  const [welfareOn, setWelfareOn] = useState<boolean>(initial.welfarePrice !== null);
  const [bizOnly, setBizOnly] = useState<boolean>(initial.bizOnly);
  ...
  const [detailContent, setDetailContent] = useState<string>(initial.detailContent ?? '');
  const [manager, setManager] = useState<string>(initial.manager ?? '');
  ...
  const submitProduct = async (): Promise<void> => {
    ...
    setSubmitting(true);
    try {
      const res = await fetch(`/cozycare/api/products/${initial.id}`, {
        method: 'PATCH',
        ...
        body: JSON.stringify({
          ...
          images: subImages
            .filter((s): s is { file: File | null; preview: string } => s !== null)
      ...
      if (!res.ok) {
        ...
        setSubmitError(data?.error === 'missing_fields' ? '필수 항목이 누락되었습니다.' : '저장에 실패했습니다.');
        return;
      }
      setSuccessOpen(true);
    } catch {
    ...
    <>
        ...
        <div className="reg-body" style={REG_BODY_STYLE}>
          ...
          <div className="reg-footer" style={REG_FOOTER_STYLE}>
            ...
            <button
              ...
              disabled={submitting}
            >
              <i className="icon-check" style={{ fontSize: 16 }}></i> {submitting ? '저장 중...' : '저장'}
            </button>
          </div>
        </div>
      </div>
      ...
      >
        <div className="modal-content" style={{ width: 400 }}>
          ...
          <div className="modal-title">수정이 완료되었습니다</div>
