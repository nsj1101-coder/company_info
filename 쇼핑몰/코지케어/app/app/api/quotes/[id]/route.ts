...
type PatchBody = { status?: string; memo?: string };

const STATUS_MAP: Record<string, QuoteStatus> = {
  requested: QuoteStatus.requested,
...
export async function PATCH(req: Request, ctx: RouteContext) {
  ...
  await prisma.quote.update({
    where: { id },
    data: { status: status ?? existing.status, memo: body.memo !== undefined ? body.memo : existing.memo },
  });
